...
Dotori for Document(이하 Dotori)의 RAG(검색 증강 생성) 기능은 로컬 LLM 서버를 사용합니다.
...
# 확인 프롬프트 없이 즉시 실행
python install.py --remove-llm --yes
```
...
```bash
docker compose exec app python manage.py llm_model_catalog show <모델-id>
```
