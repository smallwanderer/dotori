...
# Only the process gunicorn boots as dotori-document's model owner (see
# DOTORI_EMBEDDING_MODEL_PROCESS in docker-compose.yml) gets the tiny internal
# urlconf (livez/readyz/embed). Every other process -- app, and dotori-document's
# own Celery worker -- uses the normal user-facing urlconf, which no longer
# defines any internal embedding path at all.
ROOT_URLCONF = (
    'config.embedding_urls'
    if os.getenv("DOTORI_EMBEDDING_MODEL_PROCESS") == "1"
    else 'config.urls'
)
...
# app/dotori-document 컨테이너가 같은 LOG_DIR을 공유하므로, 서비스별로 로그 파일을
...
# Shared secret app <-> dotori-document send on the internal /embed call.
