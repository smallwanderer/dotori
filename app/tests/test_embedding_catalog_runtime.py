...
def test_only_verified_bge_m3_profile_is_supported():
    supported = get_supported_embedding_catalog()

    assert [entry.id for entry in supported] == ["bge-m3-hybrid"]
    entry = supported[0]
    assert entry.repo_id == "BAAI/bge-m3"
    assert entry.revision == "5617a9f"
    assert entry.provider == "bgem3_hybrid"
    assert entry.store == "pgvector_chunk_1024"
    assert entry.dimension == 1024
    assert entry.supports_sparse is True
