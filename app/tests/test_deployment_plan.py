...
def test_fast_start_command_explicitly_disables_builds():
    ...
    assert command == (
        "docker compose -f docker-compose.yml up --no-build -d db redis app"
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            "basic",
            (),
            ("embedding-worker", "search-worker", "rag-worker", "recovery-worker"),
        ),
        (
            "search",
            ("embedding-worker", "search-worker", "recovery-worker"),
            ("rag-worker",),
        ),
    ],
)
...
def test_rag_worker_is_enabled_only_after_runtime_is_resolved(tmp_path):
    pending = build_deployment_plan("1")
    active = build_deployment_plan("rag", runtime=_runtime(tmp_path))

    assert "rag-worker" not in pending.enabled_services
    assert "rag-worker" in active.enabled_services
...
def test_plan_rejects_runtime_from_another_scope(tmp_path):
    runtime = _runtime(tmp_path, scope="development")
